...
<div class="ecco_lib">
		...
		<h3 class="sectionname">Welcome to the e-CCO Library!</h3>

		<div class="libary_top_search<?php if ($category_ids) echo ' float_beside_result';?>">
		<form method="get" action="adv_search.php" class="form-horizontal">
			
			<div class="form-group">
				<label for="searchtext" class="col-sm-2 control-label">Search:</label>
				<div class="col-sm-9">
					<input id="searchtext" name="q" type="text" class="libary_search_input form-control" value="<?php p($q) ?>" />
					
				</div>
				<div class="col-sm-1 searchInputIcon">
			...
			<h4 class="eccoLibSubH">What kind of content would you like to view?</h4>
			...
			<div class="row" >
				...
				<div class="col-sm-3 eccoLibKatChCtn">
					<div class="checkbox">
						<label>
							<input type="checkbox" id="search-all-categories" value="Abstracts" <?php if (count($category_ids) == 0) echo 'checked="checked"'; ?>><img src="pix/eGuide_All.png" class="searchLibCheckIcon" /><span class="searchLibCheckTxt">All</span>
						</label>
					</div>
					<?php
					foreach ($categoryManager->getChildren(51002) as $category) {
						echo '<div class="checkbox"><label><input type="checkbox" name="category_ids[]" value="'.$category->id.'" ';
							if(in_array($category->id, $category_ids)) { echo 'checked="checked"';$result_filter_summary->ctype.=' ,'.$category->name;}
							echo '/><img src="pix/contenttypicon'.$category->id.'.png" class="searchLibCheckIcon" /><span class="searchLibCheckTxt">'.$category->name.'</span></label></div>';
					}
				...
				<div class="col-sm-5 form-horizontal">
					<?php
						...
						echo '<div class="form-group"><label for="" class="col-sm-3 control-label">Year:</label><div class="col-sm-9"> ';
						echo html_writer::select($years, 'filter_year', $filter_year, false, array('class'=>'form-control'));
						echo '</div></div>';
						
						$values = array_map(function($cat) { return $cat->name; }, $categoryManager->getChildren(51001));
						echo '<div class="form-group"><label for="" class="col-sm-3 control-label">Category:</label><div class="col-sm-9"> ';
						echo html_writer::select($values, 'filter_category', $filter_category, false, array('class'=>'form-control'));
						if ($filter_category) {
						...
						echo '</div></div>';
						if ($values) {
							echo '<div class="form-group"><label for="" class="col-sm-3 control-label">Sub Type:</label><div class="col-sm-9"> ';
							...
							echo html_writer::select($values, 'filter_sub_type', $filter_sub_type, false, array('class'=>'form-control'));
							if ($filter_sub_type) {
				...
				<div class="col-sm-4">
					...
					<div class="form-group">
						<div class="col-sm-12">
							<input value="Keywords" name="keywords" type="submit" class="form-control">
						</div>
					</div>
					<?php if (is_dir(__DIR__.'/../../mod/library')) { ?>
							<div class="form-group">
								<div class="col-sm-12">
									<input value="Archive 2011-2015" type="button" class="form-control" onclick="document.location.href='<?php echo $CFG->wwwroot.'/mod/library/adv_search.php'; ?>';">
								</div>
							</div>
							<?php } ?>
			...
			<div class="row bottomSearchBar">
				<div class="col-sm-12">
					<input value="Clear Filter" type="button" class="clear-filter btn-seFo" onclick="document.location.href='<?php echo $_SERVER['PHP_SELF']; ?>';">
