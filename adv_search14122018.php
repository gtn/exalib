...
echo '<div class="ecco_lib">';
		...
		if ($resulttrue==false)	echo $filtercontent->form;
			$filtercontent->searchbar='<div class="form-group">
				...
				<div class="col-sm-9">
					<input id="searchtext" name="q" type="text" class="libary_search_input form-control" value="'.p($q).'" />
		...
		if ($resulttrue==false) {
			...
			$filtercontent->contenttype='
				...
				<div class="col-sm-3 eccoLibKatChCtn">
					...
					foreach ($categoryManager->getChildren(51002) as $category) {
						...
						if(in_array($category->id, $category_ids)) { $filtercontent->contenttype.='checked="checked"';$result_filter_summary->ctype.=' ,'.$category->name;}
			...
			if ($resulttrue==false) echo '<div class="col-sm-5 form-horizontal">';
						...
						$filtercontent->year.= html_writer::select($years, 'filter_year', $filter_year, false, array('class'=>'form-control'));
			...
			if ($resulttrue==false) echo $filtercontent->year;
						...
						$filtercontent->cat.= html_writer::select($values, 'filter_category', $filter_category, false, array('class'=>'form-control'));
						if ($filter_category) {
						...
						if ($values) {
							...
							$filtercontent->cat.= html_writer::select($values, 'filter_sub_type', $filter_sub_type, false, array('class'=>'form-control'));
							if ($filter_sub_type) {
				...
				if (is_dir(__DIR__.'/../../mod/library')) {
								$filtercontent->archivebutton='<div class="form-group">
									<div class="col-sm-12">
										<input value="Archive 2011-2015" type="button" class="form-control" onclick="document.location.href=\''.$CFG->wwwroot.'/mod/library/adv_search.php\';">
									</div>
								</div>';
								if ($resulttrue==false){
...
<div style="margin-top: 30px;">
	<div class="row">
		...
		echo '<div class="form-group">
						<div class="">
			<input value="Search" type="button" class="form-control" onclick="document.location.href=\''.$_SERVER['PHP_SELF'].'\';">
			</div>
					</div>';
			echo $filtercontent->archivebutton;
				echo '<div class="form-group">
